...
public class MonitorViewModel : ViewModelBase
{
	...
	private void LoadCustomization() => _controller.TryLoadCustomization(DeviceInstanceId, ref _name, ref _isUnison, ref _rangeLowest, ref _rangeHighest, ref _brightenHotkey, ref _darkenHotkey, ref _cycleInputHotkey);
	private void SaveCustomization() => _controller.SaveCustomization(DeviceInstanceId, _name, _isUnison, _rangeLowest, _rangeHighest, _brightenHotkey, _darkenHotkey, _cycleInputHotkey);
	...
	public HotkeyItem BrightenHotkey
	{
		get => _brightenHotkey ??= new HotkeyItem();
		set
		{
			if (SetProperty(ref _brightenHotkey, value))
				SaveCustomization();
		}
	}
	private HotkeyItem _brightenHotkey;

	public HotkeyItem DarkenHotkey
	{
		get => _darkenHotkey ??= new HotkeyItem();
		set
		{
			if (SetProperty(ref _darkenHotkey, value))
				SaveCustomization();
		}
	}
	private HotkeyItem _darkenHotkey;

	public HotkeyItem CycleInputHotkey
	{
		get => _cycleInputHotkey ??= new HotkeyItem();
		set
		{
			if (SetProperty(ref _cycleInputHotkey, value))
				SaveCustomization();
		}
	}
	private HotkeyItem _cycleInputHotkey;
	...
	public bool CycleInputSource()
	{
		var (success, data) = GetValue(0x60); // Input Source
		if (success == true && data != null)
		{
			int nextValue = (int)((data.Value % 4) + 1); 
			var (setSuccess, _) = SetValue(0x60, nextValue);
